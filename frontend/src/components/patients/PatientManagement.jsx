...
export function PatientManagement() {
  ...
  const [formData, setFormData] = useState({
    ...
    status: 'active', patientType: 'consultancy'
  });
...
const handleStatusChange = async (patientId, newStatus) => {
  try {
    ...
    const dob = patient.dateOfBirth ? new Date(patient.dateOfBirth).toISOString().split('T')[0] : null;

    const response = await fetch(`http://localhost:5001/api/patients/${patientId}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ ...patient, status: newStatus, dateOfBirth: dob })
    });
              ...
              {filteredPatients.map(patient => (
                <div key={patient.id} className="border rounded-lg p-4 flex justify-between items-center hover:bg-gray-50 transition">
                  ...
                  <div className="flex gap-2">
                    ...
                    {/* Status Button */}
                    <Select value={patient.status || 'active'} onValueChange={(value) => handleStatusChange(patient.id, value)}>
                      <SelectTrigger>
