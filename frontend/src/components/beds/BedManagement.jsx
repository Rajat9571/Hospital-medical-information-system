...
import { Alert, AlertDescription } from '../ui/alert';
import { toast } from 'sonner';
import { Bed, User, Plus, MapPin, Clock, AlertTriangle, CheckCircle, Wrench, Search, UserPlus, UserX, Activity, Settings, Stethoscope } from 'lucide-react';
import { useAuth } from '../../context/AuthContext';
...
const [assignmentData, setAssignmentData] = useState({ patient_id: '', priority: 'normal' });
  ...
  // Fetch beds and active patients
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const bedsRes = await axios.get('http://localhost:5001/api/beds');
        const patientsRes = await axios.get('http://localhost:5001/api/beds/unassigned-patients');
        setBeds(bedsRes.data);
        setPatients(patientsRes.data);
      } catch (err) {
        console.error(err);
        toast.error('Failed to fetch beds or patients');
      } finally {
        setLoading(false);
      }
    };
  ...
  const getBedTypeIcon = (bedType) => {
    switch (bedType) {
      case 'icu': return <AlertTriangle className="h-4 w-4" />;
      case 'emergency': return <Activity className="h-4 w-4" />;
      case 'private': return <Bed className="h-4 w-4" />;
      default: return <Bed className="h-4 w-4" />;
    }
  };
  ...
  const filteredBeds = beds.filter(bed => {
    const matchesSearch =
      ...
      (bed.room_number?.toLowerCase() || '').includes(searchTerm.toLowerCase());
    const matchesWard = filterWard === 'all' || bed.ward === filterWard;
    const matchesStatus = filterStatus === 'all' || bed.status === filterStatus;
    return matchesSearch && matchesWard && matchesStatus;
  });
  ...
  const handleAssignBed = async (e) => {
    e.preventDefault();
    if (!selectedPatient || !selectedBed) return toast.error('Select patient and bed');
    try {
      const res = await axios.put(`http://localhost:5001/api/beds/${selectedBed.id}/assign`, {
        patient_id: selectedPatient.id,
        assigned_doctor: user?.name
      });

      setBeds(prev =>
        prev.map(b => b.id === selectedBed.id ? res.data : b)
      );
      setPatients(prev => prev.filter(p => p.id !== selectedPatient.id));

      toast.success(`Bed ${selectedBed.number} assigned successfully`);
      setShowAssignDialog(false);
      setSelectedBed(null);
...
// const handleDischargeBed = (bed) => {
//   // Immediately set bed to cleaning and remove patient info
...
//       maintenance_reason: 'Post-discharge cleaning' 
//     } : b
//   ));

//   // Clear any selected bed so the UI doesn't show old info
//   setSelectedBed(null);

//   toast.success(`Patient discharged from bed ${bed.number}. Bed set for cleaning.`);

//   // After 2 seconds, set bed to available
//   setTimeout(() => 
//     setBeds(prev => prev.map(b => 
//       b.id === bed.id ? { ...b, status: 'available', maintenance_reason: null } : b
//     )), 
//   2000);
// };
  ...
  setBeds(prev => prev.map(b =>
    b.id === bed.id
      ? { 
          ...b,
          status: 'cleaning',
          patient: null,        // <-- clear patient object
          patient_id: null,
          patient_name: null,
          admission_date: null,
          assigned_doctor: null,
...
useEffect(() => {
  const interval = setInterval(() => {
    setBeds(prev =>
      prev.map(b =>
        b.status === 'cleaning' 
          ? { ...b, status: 'available', maintenance_reason: null } 
          : b
      )
    );
  ...
  const handleMaintenanceToggle = async (bed) => {
    try {
      const res = await axios.put(`http://localhost:5001/api/beds/${bed.id}/maintenance`);
      setBeds(prev =>
        prev.map(b => b.id === bed.id ? res.data : b)
      );
      toast.success(`Bed ${bed.number} ${res.data.status === 'maintenance' ? 'set for maintenance' : 'back in service'}`);
    } catch (err) {
      console.error(err);
  ...
  const handleAddBed = async (e) => {
    e.preventDefault();
    if (!newBedData.number || !newBedData.ward) return toast.error('Please fill required fields');
    try {
      const res = await axios.post('http://localhost:5001/api/beds', newBedData);
  ...
  return (
    <div className="space-y-6">
          ...
          <Dialog open={showAddBedDialog} onOpenChange={setShowAddBedDialog}>
            ...
            <DialogContent>
              ...
              <form onSubmit={handleAddBed} className="space-y-4">
                ...
                <div className="flex justify-end gap-2">
                  <Button type="button" variant="outline" onClick={() => setShowAddBedDialog(false)}>Cancel</Button>
                  <Button type="submit">Add Bed</Button>
                </div>
              </form>
            </DialogContent>
      ...
      {/* Stats Cards */}
      <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
        <Card><CardContent className="p-4"><div className="flex items-center"><div className="flex-1"><p className="text-sm text-gray-600">Total Beds</p><p className="text-2xl font-semibold text-gray-900">{beds.length}</p></div><Bed className="h-8 w-8 text-blue-600" /></div></CardContent></Card>
        <Card><CardContent className="p-4"><div className="flex items-center"><div className="flex-1"><p className="text-sm text-gray-600">Available</p><p className="text-2xl font-semibold text-green-700">{availableBeds}</p></div><CheckCircle className="h-8 w-8 text-green-600" /></div></CardContent></Card>
        <Card><CardContent className="p-4"><div className="flex items-center"><div className="flex-1"><p className="text-sm text-gray-600">Occupied</p><p className="text-2xl font-semibold text-red-700">{occupiedBeds}</p></div><User className="h-8 w-8 text-red-600" /></div></CardContent></Card>
        <Card><CardContent className="p-4"><div className="flex items-center"><div className="flex-1"><p className="text-sm text-gray-600">Maintenance</p><p className="text-2xl font-semibold text-yellow-700">{maintenanceBeds}</p></div><Wrench className="h-8 w-8 text-yellow-600" /></div></CardContent></Card>
        <Card><CardContent className="p-4"><div className="flex items-center"><div className="flex-1"><p className="text-sm text-gray-600">Cleaning</p><p className="text-2xl font-semibold text-blue-700">{cleaningBeds}</p></div><Activity className="h-8 w-8 text-blue-600" /></div></CardContent></Card>
      </div>

      {/* Ward Stats */}
      <Card>
        <CardHeader><CardTitle>Ward Occupancy Overview</CardTitle></CardHeader>
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            {wardStats.map((ward) => (
              <div key={ward.name} className="border rounded-lg p-4">
                ...
                <div className="space-y-1">
                  <div className="flex justify-between text-sm"><span className="text-gray-600">Total:</span><span className="font-medium">{ward.total}</span></div>
                  <div className="flex justify-between text-sm"><span className="text-green-600">Available:</span><span className="font-medium">{ward.available}</span></div>
                  <div className="flex justify-between text-sm"><span className="text-red-600">Occupied:</span><span className="font-medium">{ward.occupied}</span></div>
                </div>
                <div className="mt-2 w-full bg-gray-200 rounded-full h-2">
                  <div className="bg-red-600 h-2 rounded-full transition-all" style={{ width: `${ward.occupancy}%` }}></div>
                </div>
              </div>
            ))}
          </div>
      ...
      <Card>
        <CardContent className="p-4">
          <div className="flex flex-col md:flex-row gap-4">
            <div className="relative flex-1">
              <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 h-4 w-4" />
              <Input placeholder="Search beds..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="pl-10"/>
            </div>
            <div className="flex gap-2">
              <Select value={filterWard} onValueChange={setFilterWard}>
      ...
      {/* Beds Grid */}
      <Card>
        <CardHeader><CardTitle className="flex items-center justify-between"><span>Bed Status Overview</span><Badge variant="outline">{filteredBeds.length} of {beds.length} beds</Badge></CardTitle></CardHeader>
        <CardContent>
          {loading ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
              {[...Array(8)].map((_, i) => <div key={i} className="animate-pulse border rounded-lg p-4"><div className="h-4 bg-gray-200 rounded w-1/4 mb-2"></div><div className="h-3 bg-gray-200 rounded w-1/3 mb-2"></div><div className="h-3 bg-gray-200 rounded w-1/2"></div></div>)}
            </div>
          ) : filteredBeds.length === 0 ? (
            <div className="text-center py-8">
              <Bed className="h-12 w-12 text-gray-400 mx-auto mb-4" />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
              {filteredBeds.map((bed) => (
                <div key={bed.id} className={`border rounded-lg p-4 hover:shadow-md transition-all cursor-pointer ${bed.status === 'available' ? 'border-green-200 bg-green-50 hover:bg-green-100' : bed.status === 'occupied' ? 'border-red-200 bg-red-50 hover:bg-red-100' : bed.status === 'maintenance' ? 'border-yellow-200 bg-yellow-50 hover:bg-yellow-100' : 'border-blue-200 bg-blue-50 hover:bg-blue-100'}`}>
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex items-center gap-2">
                      <div className={`p-1.5 rounded-full ${getBedTypeColor(bed.bed_type)}`}>{getBedTypeIcon(bed.bed_type)}</div>
                      <div>
                        <span className="font-semibold text-gray-900">Bed {bed.number}</span>
                        {bed.room_number && <p className="text-xs text-gray-500">{bed.room_number}</p>}
                      </div>
                    </div>
                    <Badge className={`${getStatusColor(bed.status)} text-xs`}><span className="flex items-center gap-1">{getStatusIcon(bed.status)}{bed.status}</span></Badge>
                  </div>

                  <div className="space-y-2 mb-4">
                    <div className="flex items-center gap-2 text-sm"><MapPin className="h-3 w-3 text-gray-400" /><span className="text-gray-600">{bed.ward} Ward</span></div>
                    {bed.patient && <div className="flex items-center gap-2 text-sm"><User className="h-3 w-3 text-gray-400" /><span className="text-gray-900 font-medium">{bed.patient.name}</span></div>}
                    {bed.admission_date && <div className="flex items-center gap-2 text-sm"><Clock className="h-3 w-3 text-gray-400" /><span className="text-gray-600">Admitted {new Date(bed.admission_date).toLocaleDateString()}</span></div>}
                    {bed.assigned_doctor && <div className="flex items-center gap-2 text-sm"><Stethoscope className="h-3 w-3 text-gray-400" /><span className="text-gray-600">{bed.assigned_doctor}</span></div>}
                    {bed.maintenance_reason && <div className="flex items-center gap-2 text-sm"><AlertTriangle className="h-3 w-3 text-yellow-500" /><span className="text-gray-600">{bed.maintenance_reason}</span></div>}
                  </div>
    ...
    <DialogContent>
      ...
      <form onSubmit={handleAssignBed} className="space-y-4">
        ...
        <div>
          <Label>Select Patient</Label>
          <Select
            value={assignmentData.patient_id}
            onValueChange={(value) =>{
              setAssignmentData({ ...assignmentData, patient_id: value });
              const patient = patients.find(p => String(p.id) === value);
              setSelectedPatient(patient); // <-- set selectedPatient here
            }}
          >
            <SelectTrigger>
              <SelectValue placeholder="Choose a patient" />
        ...
        <div className="flex justify-end gap-2">
          ...
          <Button type="submit" disabled={!assignmentData.patient_id}>
            Assign Patient
                    ...
                    {bed.status === 'occupied' && <Button variant="destructive" size="sm" className="flex-1" onClick={() => handleDischargeBed(bed)}><UserX className="h-3 w-3 mr-1" />Discharge</Button>}
                    {(bed.status === 'maintenance' || bed.status === 'available') && <Button variant="outline" size="sm" className="flex-1" onClick={() => handleMaintenanceToggle(bed)}><Settings className="h-3 w-3 mr-1" />{bed.status === 'maintenance' ? 'End Maintenance' : 'Maintenance'}</Button>}
                  </div>
                </div>
              ))}
            </div>
