...
//   const demoUsers = [
//     { email: 'admin@hospital.com', password: 'admin123', name: 'Admin User', role: 'admin', department: 'administration' },
//     { email: 'doctor@hospital.com', password: 'doctor123', name: 'Dr. Smith', role: 'doctor', department: 'cardiology' },
//     { email: 'nurse@hospital.com', password: 'nurse123', name: 'Nurse Jane', role: 'nurse', department: 'emergency' },
//     { email: 'staff@hospital.com', password: 'staff123', name: 'Staff Member', role: 'staff', department: 'general' }
//   ];
...
// //       'INSERT INTO users (email, password, name, role, department) VALUES (?, ?, ?, ?, ?)',
// //       [email.toLowerCase(), hashed, name, role.toLowerCase(), department || null]
// //     );

// //     res.status(201).json({
// //       success: true,
// //       message: 'User registered successfully',
// //       user: { id: result.insertId, email: email.toLowerCase(), name, role: role.toLowerCase(), department }
// //     });
// //   } catch (err) {
...
//       return res.status(400).json({ error: 'User already exists or pending approval' });
...
//   const { id } = req.params;
//   try {
//     const [rows] = await pool.execute('SELECT * FROM pending_registrations WHERE id = ?', [id]);
...
//     const [result] = await pool.execute(
//       'INSERT INTO users (email, password, name, role, department) VALUES (?, ?, ?, ?, ?)',
//       [request.email, request.password, request.name, request.role, request.department]
//     );
...
//   try {
//     await pool.execute(
//       'UPDATE pending_registrations SET status = "rejected", rejection_reason = ?, rejected_at = NOW() WHERE id = ?',
...
//     res.json({ success: true, message: 'Login successful', token, user: { id: user.id, email: user.email, name: user.name, role: user.role, department: user.department } });
//   } catch (err) {
